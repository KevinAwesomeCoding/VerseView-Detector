...
class VerseViewApp(ctk.CTk):
    def __init__(self):
        ...
        self._restore_dialog_shown = False   # prevent double-fire of restore dialog
    ...
    def _start(self):
        try:
            ...
            self._start_session_autosave()   # start 60-s periodic session save
    ...
    def _clear_sermon_memory(self):
        if mb.askyesno("Clear Memory", "Are you sure you want to delete the current recorded sermon?\n\nThis cannot be undone!"):
            ...
            session_state.clear_session()   # wipe session so a stale restore is never offered
    ...
    def _restore_session_data(self, data: dict):
        ...
        try:
            ...
            # Session is now live in memory — clear the file.
            # Periodic auto-save will recreate it within 60 s once engine starts.
            session_state.clear_session()
    ...
    def _build_session_data(self) -> dict:
        """Collect current in-memory state into a session dict. No API keys."""
        return {
            ...
            "last_explicit_ref_time":    0.0,  # saved as 0 so it never expires on restore
            "notes_saved":               self._notes_saved,
            "notes_generated":           self._notes_generated,
        }

    def _start_session_autosave(self):
        """Start a 60-second background daemon thread that auto-saves the session."""
