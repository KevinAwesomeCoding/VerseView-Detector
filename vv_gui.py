...
def _build_options(self):
    ...
    # ── Manual keyer test toggle ──
    self._atem_keyer_manual_on = False
    ...
    self.atem_test_btn.grid(row=r, column=0, sticky="ew", padx=10, pady=(0, 8))
    r += 1
...
def _toggle_atem_keyer_manual(self):
    """Manually toggle the ATEM upstream keyer ON or OFF for testing."""
    self._atem_keyer_manual_on = not self._atem_keyer_manual_on

    if self._atem_keyer_manual_on:
        # Turn ON
        self.atem_test_btn.configure(
            text="🟢  Keyer: ON",
            fg_color="#1a6b1a",
            hover_color="#145214",
            text_color="white",
        )
        self._append_log("🎬 ATEM: manual keyer ON")
        def _fire_on():
            try:
                ...
                self._atem_sw = sw   # keep reference so we can turn it off
                self._append_log(f"🎬 ATEM: keyer switched ON ({ip})")
            except Exception as e:
                self._append_log(f"⚠️ ATEM error: {e}")
                self.after(0, lambda: self._reset_atem_test_btn())
        import threading
        threading.Thread(target=_fire_on, daemon=True).start()

    else:
        # Turn OFF
        self._reset_atem_test_btn()
        self._append_log("🎬 ATEM: manual keyer OFF")
        def _fire_off():
            try:
                import time as _t
                sw = getattr(self, "_atem_sw", None)
                if sw:
                    sw.setKeyerOnAirEnabled(0, 0, False)
                    _t.sleep(0.5)
                    sw.disconnect()
                    self._atem_sw = None
                    self._append_log("🎬 ATEM: keyer switched OFF")
            except Exception as e:
                self._append_log(f"⚠️ ATEM error: {e}")
        import threading
        threading.Thread(target=_fire_off, daemon=True).start()

def _reset_atem_test_btn(self):
    """Reset the test button to OFF state visually."""
    self._atem_keyer_manual_on = False
    self.atem_test_btn.configure(
        text="⬛  Keyer: OFF",
        fg_color=("gray75", "gray25"),
