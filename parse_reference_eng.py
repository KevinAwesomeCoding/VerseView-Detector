    ...
    while i < len(words):
        ...
        if i + 1 < len(words) and w in NUMBER_MAP and words[i + 1] == "hundred":
            ...
            if i + 2 < len(words) and words[i + 2] in NUMBER_MAP:
                ...
                i += 2
            continue
        # "hundred [nineteen]" / "hundred [fifty seven]" → 119 / 157
        # Handles STT like "Psalms hundred nineteen verses fifty seven" → Psalm 119:57.
        # The "two hundred" branch above only fires when a multiplier word precedes "hundred";
        # this branch covers a bare leading "hundred" with no multiplier word before it.
        if w == "hundred":
            base = 100
            if i + 1 < len(words) and words[i + 1] in NUMBER_MAP:
                addend = int(NUMBER_MAP[words[i + 1]])
                # "hundred fifty seven" → 157: tens word followed by ones word
                if (addend >= 20 and addend % 10 == 0
                        and i + 2 < len(words) and words[i + 2] in NUMBER_MAP
                        and 1 <= int(NUMBER_MAP[words[i + 2]]) <= 9):
                    result.append(str(base + addend + int(NUMBER_MAP[words[i + 2]])))
                    i += 3
                else:
                    result.append(str(base + addend))
                    i += 2
            else:
                result.append("100")
                i += 1
