...
class LocalWhisperProvider(STTProvider):
    ...
    async def stream_audio(
        ...
        # Resolve the POST URL.  If the caller already specified a full path
        # (e.g. http://127.0.0.1:8080/transcribe) use it as-is; otherwise
        # append /transcribe to the base URL.
        from urllib.parse import urlparse as _urlparse
        _parsed = _urlparse(endpoint)
        if _parsed.path and _parsed.path.rstrip("/") not in ("", "/"):
            post_url = endpoint
        else:
            post_url = f"{endpoint}/transcribe"
        ...
        try:
            while not stop_event.is_set():
                ...
                try:
                    ...
                    # Pass language hint if the server accepts it.
                ...
                # Accept both the simple {"text": "..."} shape and the
                # OpenAI-compatible {"choices": [{"text": "..."}]} shape.
                sentence = ""
                if isinstance(raw_json, dict):
                    sentence = (
                        raw_json.get("text")
                        or (
                            (raw_json.get("choices") or [{}])[0].get("text", "")
